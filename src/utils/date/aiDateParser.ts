 ...
 */
export async function parseWithAI(input: string, aiModelSessionFactory?: AIModelSessionFactory): Promise<DayRange> {
    try {
        ...
        try {
            const parsed = JSON.parse(response) as DayRange;
            if (
